...
import { getTeachers } from "../services/teacherService";
import { getFeeReport } from "../services/reportsService";
import { getMeritList, getExams } from "../services/examService";
    ...
    setTimeout(async () => {
      try {
        ...
        if (reportType === "admissions") {
          const res = await getStudents();
          formattedData = res.data.map((s: any) => ({
            col1: s.admissionNo || "N/A",
            col2: s.user?.name || "N/A",
            col3: `${s.class?.name || "Class"} - ${s.section?.name || "Section"}`,
            col4: s.parentPhone || "N/A"
          }));
        }
          ...
          const res = await getTeachers();
          formattedData = res.data.map((t: any) => ({
            col1: t.employeeId || "N/A",
            col2: t.user?.name || "N/A",
            col3: t.qualification || "N/A",
            col4: `Rs. ${t.salary || 0}`
          }));
        }
          ...
          const res = await getFeeReport(filterMonth, undefined);
          formattedData = res.data.map((inv: any) => {
            const formattedMonth = inv.month 
              ? new Date(inv.month).toLocaleDateString(undefined, { month: "long", year: "numeric" }) 
              : "N/A";
            return {
              col1: inv.student?.admissionNo || "N/A",
              col2: inv.student?.user?.name || "N/A",
              col3: `${inv.class?.name || "Active Class"} (${formattedMonth})`,
              col4: `Rs. ${inv.totalAmount || 0} [${(inv.status || "UNPAID").toUpperCase()}]`
            };
          });
        }
          ...
          formattedData = res.meritList.map((m: any, index: number) => ({
            col1: `${index + 1} Position`,
            col2: m.name || "N/A",
            col3: m.admissionNo || "N/A",
            col4: `${(m.percentage || 0).toFixed(2)}% Marks Obtained`
          }));
        }
