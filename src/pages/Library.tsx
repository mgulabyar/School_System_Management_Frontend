...
import { addBook, getBooks, issueBook, returnBook, getIssuedBooks } from '../services/libraryService';
  ...
  const loadBaselineData = useCallback(async () => {
    try {
      ...
      const activeStudents = resStudents.data
        ...
        .map((s: any) => ({ _id: s.user._id, name: s.user.name, role: 'Student' }));

      const activeTeachers = resTeachers.data
        .filter((t: any) => t.status === 'Active')
        .map((t: any) => ({ _id: t.user._id, name: t.user.name, role: 'Teacher' }));
  ...
  return (
    ...
    >
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 4, fontSize: '0.925rem', fontFamily: '"Roboto", "Arial", sans-serif' }}>
        Manage library inventory, issue books, track return timelines, and record overdue fines.
      </Typography>
        ...
        <Card sx={{ borderRadius: "10px", bgcolor: 'background.paper', boxShadow: mode === 'light' ? '0 1px 3px rgba(15, 23, 42, 0.04)' : 'none', border: mode === 'dark' ? '1px solid #334155' : '1px solid #CBD5E1', maxWidth: 600 }}>
          <CardContent sx={{ p: { xs: 2.5, sm: 3.5 } }}>
            ...
            <form onSubmit={handleAddBook}>
              ...
              >
                {addBookLoading ? <CircularProgress size={18} color="inherit" /> : 'Add Book'}
              </Button>
            </form>
          </CardContent>
        </Card>
            ...
            <form onSubmit={handleIssueBook}>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, mb: 3.5 }}>
                ...
                <FormControl size="small" fullWidth>
                  ...
                  >
                    {books.filter(b => b.available > 0).map((b) => (
                      <MenuItem key={b._id} value={b._id} sx={{ fontFamily: '"Roboto", "Arial", sans-serif', fontSize: "13px" }}>
                ...
                <FormControl size="small" fullWidth>
                  ...
                  >
                    {borrowers.map((borr) => (
                      <MenuItem key={borr._id} value={borr._id} sx={{ fontFamily: '"Roboto", "Arial", sans-serif', fontSize: "13px" }}>
                        {borr.name} ({borr.role})
                      </MenuItem>
                    ))}
                  </Select>
              ...
              <Button 
                ...
                sx={{ height: 42, fontSize: "13px", borderRadius: "8px", textTransform: "none", boxShadow: "none", fontWeight: 600, fontFamily: '"Roboto", "Arial", sans-serif' }}
              >
                {issueLoading ? <CircularProgress size={18} color="inherit" /> : 'Issue Book'}
              </Button>
            </form>
          </CardContent>
        </Card>
                    ...
                    <TableContainer component={Paper} elevation={0} sx={{ bgcolor: 'transparent' }}>
                      <Table sx={{ minWidth: 800 }}>
                        <TableHead sx={{ bgcolor: 'action.hover' }}>
                          <TableRow>
                            <TableCell sx={{ fontWeight: 700, fontSize: "13px", fontFamily: '"Roboto", "Arial", sans-serif', borderBottomColor: 'divider', whiteSpace: "nowrap" }}>Book Title</TableCell>
                            <TableCell sx={{ fontWeight: 700, fontSize: "13px", fontFamily: '"Roboto", "Arial", sans-serif', borderBottomColor: 'divider', whiteSpace: "nowrap" }}>Borrower</TableCell>
                            <TableCell sx={{ fontWeight: 700, fontSize: "13px", fontFamily: '"Roboto", "Arial", sans-serif', borderBottomColor: 'divider', whiteSpace: "nowrap" }}>Role</TableCell>
                            <TableCell sx={{ fontWeight: 700, fontSize: "13px", fontFamily: '"Roboto", "Arial", sans-serif', borderBottomColor: 'divider', whiteSpace: "nowrap" }}>Issue Date</TableCell>
                            <TableCell sx={{ fontWeight: 700, fontSize: "13px", fontFamily: '"Roboto", "Arial", sans-serif', borderBottomColor: 'divider', whiteSpace: "nowrap" }}>Due Date</TableCell>
                        ...
                        <TableBody>
                          {issuedBooks.map((record) => (
                            <TableRow key={record._id} sx={{ '&:last-child td, &:last-child th': { border: 0 }, '&:hover': { bgcolor: 'action.hover' } }}>
                              ...
                              <TableCell sx={{ fontWeight: 600, fontSize: "13px", fontFamily: '"Roboto", "Arial", sans-serif', borderBottomColor: 'divider', whiteSpace: "nowrap" }}>{record.borrower.name}</TableCell>
                              <TableCell sx={{ fontSize: "12px", fontFamily: '"Roboto", "Arial", sans-serif', textTransform: 'uppercase', borderBottomColor: 'divider', whiteSpace: "nowrap" }}>{record.borrower.role}</TableCell>
                              <TableCell sx={{ fontSize: "13px", fontFamily: '"Roboto", "Arial", sans-serif', borderBottomColor: 'divider', whiteSpace: "nowrap" }}>{new Date(record.issueDate).toLocaleDateString()}</TableCell>
                              <TableCell sx={{ fontSize: "13px", fontFamily: '"Roboto", "Arial", sans-serif', borderBottomColor: 'divider', whiteSpace: "nowrap" }}>{new Date(record.dueDate).toLocaleDateString()}</TableCell>
                              <TableCell align="right" sx={{ borderBottomColor: 'divider', whiteSpace: "nowrap" }}>
                                ...
                                >
                                  {returnLoadingId === record._id ? <CircularProgress size={14} color="inherit" /> : 'Return'}
                                </Button>
                              </TableCell>
                            </TableRow>
                          ))}
                ...
                {issuedBooks.map((record) => (
                  ...
                  >
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.8, mb: 2 }}>
                      ...
                      <Typography variant="body2" sx={{ fontFamily: '"Roboto", "Arial", sans-serif', color: 'text.secondary', fontSize: '12px' }}>
                        <strong>Issue Date:</strong> {new Date(record.issueDate).toLocaleDateString()}
                      </Typography>
                    </Box>
